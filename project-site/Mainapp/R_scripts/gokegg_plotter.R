go_enricher <- function (gokegg_dataframe, filename_id, bg_annot){
    ...
    gene_df_GOannot_sorted$pvalue = as.numeric(gene_df_GOannot_sorted$pvalue)
    ...
    plt_GO_enrich = ggplot(data_pltgoenrich, aes(x=GOterm,y=Gene_Ratio)) +
      ...
      scale_x_discrete(labels = function(x)  str_sub(x, 1, 40))


    ggsave(paste0("Mainapp/static/Temp_R_gokegg/", output_file_name, "_enrichment.png"),
           plot=plt_GO_enrich, dpi = 400, width=6, height=6)
    ggsave(paste0("Mainapp/static/Temp_R_gokegg/", output_file_name, "_enrichment.pdf"),
           plot=plt_GO_enrich, dpi = 400, width=6, height=6)
...
kegg_enricher <- function (gokegg_dataframe, filename_id, bg_annot){
    ...
    ggsave(paste0("Mainapp/static/Temp_R_gokegg/", output_file_name, "_enrichment.png"),
               plot=plt_KEGG_enrich, dpi = 400, width=6, height=6)
    ggsave(paste0("Mainapp/static/Temp_R_gokegg/", output_file_name, "_enrichment.pdf"),
               plot=plt_KEGG_enrich, dpi = 400, width=6, height=6)
