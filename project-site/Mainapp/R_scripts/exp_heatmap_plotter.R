exp_heatmap_plotter <- function(wkdir_name, exp_df, query_format){
  ...
  # exp_df: dataframe with col names ["gene_id", "sample_id", "exp_level", "tissue"],
  ...
  suppressMessages(suppressWarnings(if (!require('svglite')) install.packages('svglite'))) # SVG plot generation
  ...
  exp_heatmap = ggplot(data=exp_df, aes(x=gene_id, y=sample_id)) +
    geom_tile(alpha=0.7, aes(fill=exp_level), color='black', linewidth=0.1, linetype='dashed') +
    ...
    geom_text(aes(label=round(exp_level, 1), color=exp_level), size=3)+
    scale_color_gradient2(low="#c2ccd0", mid="#161823", high="black", midpoint = value_level_max/2) +
    guides(color='none',
  ...
  ggsave(paste0("Mainapp/static/Temp_R_genefam/", wkdir_name, "/exp_heatmap.png"), plot=heatmap_all, dpi = 300, width = 9, height = 5)
  ggsave(paste0("Mainapp/static/Temp_R_genefam/", wkdir_name, "/exp_heatmap.svg"), plot=heatmap_all, width = 9, height = 5)

  exp_df_cast_new = cbind(rownames(as.matrix(exp_df_cast)), as.matrix(exp_df_cast))
  write.csv(as.matrix(exp_df_cast_new), paste0("Mainapp/static/Temp_R_genefam/", wkdir_name, "/gene_member_exp.csv"), row.names = F)
