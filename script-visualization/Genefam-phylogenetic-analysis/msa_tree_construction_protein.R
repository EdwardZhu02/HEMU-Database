...
genefam_phylogenetic_analysis <- function(wkdir_name, msa_method, pairwisedist_method, phylotree_layout, bootstrap_rep){
  ...
  # Set work directory and read sequence (.fasta format)
  #seq_cluster_data = readDNAStringSet("F_partial-YABBY_sequences.fasta")
  seq_cluster_data = readAAStringSet("F_All-YABBY-protein-rm63.fasta")
  ...
  plt_msa_all = ggmsa(seq_cluster_msa@unmasked, 1, 500)
  ggsave("multiple_seq_alignment.pdf", plot=plt_msa_all, width = 49, height = 7, units = "in")
  
  
  # Heatmap =====
  #seq_cluster_dist_rescale = seq_cluster_dist
  # Normalize genomic distances
  #seq_cluster_dist_rescale[sapply(seq_cluster_dist_rescale,is.infinite)] = 1e15
  #seq_cluster_dist_rescale = log10(abs(seq_cluster_dist_rescale)+1)
  png(filename = "dist_heatmap.png", width=4000, height=4000, res=300)
  print(pheatmap(seq_cluster_dist))
  dev.off()
  #pheatmap(log2(seq_cluster_dist_rescale+1))
  # Heatmap =====
  ...
  seq_cluster_nj_bootstrap = boot.phylo(seq_cluster_nj, seq_cluster_dnabin, 
                                        func_bootstrap, quiet=F, B=as.numeric(bootstrap_rep), trees = T)
  ...
  plt_njtree = ggtree(seq_cluster_nj, size=1, layout=phylotree_layout) +
    ...
    #xlim(NA, 10) + # x axis limit
    #geom_treescale(x=9, y=2, color="#3f72af") # Scale bar
  
  # Save phylogenetic tree to static image
  ggsave("phylotree_nj.png", plot=plt_njtree, dpi = 300, width = 12, height = 7, units = "in")
  # Save editable tree format (.nwk)
  write.tree(seq_cluster_nj, file = "phylotree_nj.nwk")
}
